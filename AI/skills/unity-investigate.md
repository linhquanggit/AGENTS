...
## Procedure
1. **Restate & Reproduce**: State the symptom and expected behavior in one line each; confirm a reproduction (repro steps or observed evidence). Bound the scope to exactly the reported issue.
...
3. **Trace Minimal Path**:
   ...
   - Form a hypothesis, test it against the evidence; if disproven, widen one hop. Stop at the first authoritative `file:line` that explains the fault.
4. **Root Cause**:
...
5. **Fix & Defend**:
   - Describe the smallest fix that addresses the root cause. Where cheap, add a guard/assert/log so the fault cannot recur silently. Apply only if requested (see Permission Modes).
