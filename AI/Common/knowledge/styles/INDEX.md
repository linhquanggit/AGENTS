...
Named, reusable **UI/visual** style presets — evaluated from real projects or synthesized from recognized design languages — call one by name to apply it for the current task (e.g. "dùng style cupertino-dark"). UI style only, never code conventions (those stay in `../../context/Conventions.md`). Read this index first; open only the entry needed.
...
- [cupertino-dark](cupertino-dark.md) — dark iOS/Cupertino-style card list UI (rounded cards, pill toggle, segmented control)
