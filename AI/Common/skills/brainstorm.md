...
3. **Options**: Offer 2–3 design options with trade-offs. Reuse existing patterns/modules first.
4. **Recommend**: Pick one, bound the scope, and state acceptance criteria.
5. **Hand off**: On approval, route to `feature` (or a plan). Do not build here.

## Anti-Hallucination Guardrails
- **DO NOT** write or modify code in this skill.
- **DO NOT** over-ask — only unknowns that change the design.
- **DO NOT** invent new patterns when existing ones fit.
- **DO NOT** present options without trade-offs or a recommendation.

## Output
- **Goal & Unknowns**; **Questions** (one batch); **Options** (2–3 w/ trade-offs); **Recommendation** + acceptance criteria.
